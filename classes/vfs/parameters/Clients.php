 ...
 * @singleton
 */
final class vfs_parameters_Clients {
    /* resources */
    ...
    /**
     * @conn    vfs
     * @argv    vfs_clients    {"only": ["CID"]}
     * @enve
     */
    public function GET($conn, $argv, $_env) {
        $rs = $conn->vfs_clients($argv)->select("*");
        if ($_chk = $rs->next()) {
            $_env->data(dbm()->vfs_clients->copy("CID")->map($rs));
        }
        $_env->ctrl(false, $_chk, $_chk);

        return $_env->info(
            "CLIENT %s %s DITEMUKAN", $argv["CID"], ($_chk) ? "BERHASIL" : "TIDAK"
        );
    }
     ...
     * @rest
     * @conn    vfs
     * @argv    vfs_clients    {"slice": ["CID"]}
     * @enve
     */
    public function POST($conn, $argv, $_env) {
        try {
            $conn->begin();
            $conn->vfs_clients($argv)->insert();
            $conn->commit();
    ...
    public function PUT($conn, $argv, $_env) {
        try {
            $conn->begin();
            $conn->update(
                $conn->vfs_clients($argv)->slice("CID")
            );
            $conn->commit();
            $_env->push($this->msg[__FUNCTION__], $this->res);
        } catch(Exception $e) {
     ...
     * @rest
     * @conn    vfs
     * @argv    vfs_clients    {"copyOnly": ["CID"]}
     * @enve
     */
    public function DELETE($conn, $argv, $_env) {
        try {
            $conn->begin();
            $conn->vfs_clients($argv)->delete();
            $conn->commit();
            $_env->push($this->msg[__FUNCTION__], $this->res);
        } catch(Exception $e) {
            $conn->rollback();
